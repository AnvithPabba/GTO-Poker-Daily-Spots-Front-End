...
describe("HandSelectionModal", () => {
  it("offers only unblocked API-provided exact combos and saves a complete allocation", () => {
    ...
    expect(screen.getByRole("button", { name: /AhAs · featured/ })).toBeDisabled();
    expect(screen.getByRole("button", { name: "AdAs" })).toBeEnabled();

    // Act
    fireEvent.click(screen.getByRole("button", { name: "AdAs" }));
    fireEvent.click(screen.getByRole("button", { name: "Save AdAs" }));
